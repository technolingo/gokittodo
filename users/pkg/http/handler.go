...
// makeRetrieveHandler creates the handler logic
func makeRetrieveHandler(m *http.ServeMux, endpoints endpoint.Endpoints, options []http1.ServerOption) {
	m.Handle("/retrieve", http1.NewServer(endpoints.RetrieveEndpoint, decodeRetrieveRequest, encodeRetrieveResponse, options...))
}

// decodeRetrieveRequest is a transport/http.DecodeRequestFunc that decodes a
// JSON-encoded request from the HTTP request body.
func decodeRetrieveRequest(_ context.Context, r *http.Request) (interface{}, error) {
	req := endpoint.RetrieveRequest{}
	err := json.NewDecoder(r.Body).Decode(&req)
	return req, err
}

// encodeRetrieveResponse is a transport/http.EncodeResponseFunc that encodes
// the response as JSON to the response writer
func encodeRetrieveResponse(ctx context.Context, w http.ResponseWriter, response interface{}) (err error) {
	if f, ok := response.(endpoint.Failure); ok && f.Failed() != nil {
		ErrorEncoder(ctx, f.Failed(), w)
		return nil
	}
